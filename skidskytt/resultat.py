...
def get_tavlingar():
        ...
        cursor.execute(
            "SELECT id, tavling, gren, kon, GROUP_CONCAT(namn, ', ') AS topp_3 "
            "FROM resultat "
            "WHERE placering <= 3 "
            "GROUP BY id"
        )
...
def display_resultat(tavling_id, tavling_namn, tavling_gren, topp_3):
...
def get_skidskyttar():
    ...
    cursor.execute("SELECT id, namn, nation, alder FROM skidskyttar order by namn ASC")
    return cursor.fetchall()

def display_skidskytt_resultat(skidskytt_id, skidskytt_namn):
    """Displays the resultat table for the specified skidskytt."""
    skidskytt_data = cursor.execute("SELECT namn, nation, alder FROM skidskyttar WHERE id = ?", (skidskytt_id,)).fetchone()
    skidskytt_nation = skidskytt_data[1]
    skidskytt_alder = skidskytt_data[2]
    while True:
        cursor.execute(
            "SELECT DISTINCT id, tavling, gren, placering, straffrundor_totalt, total_tid "
        ...
        # Print the header
        print(f"{skidskytt_namn} ({skidskytt_alder}), {skidskytt_nation}.")
        ...
        # Print the table
        print(table)
def display_vpoang(kon):
    """Displays the current vpoang standings for the specified gender."""
    if kon not in ["m", "f"]:
        print("Felaktigt kön. Tillåtna värden är 'm' och 'f'.")
        return

    # Get all skidskyttar with the specified gender and their current vpoang
    skidskyttar = cursor.execute(
        "SELECT id, namn, vpoang FROM skidskyttar WHERE kon = ? ORDER BY vpoang DESC",
        (kon,)
    ).fetchall()
    ...
    for i, skidskytt in enumerate(skidskyttar):
        table.add_row([i+1, skidskytt[1], skidskytt[2]])
...
while True:
    ...
    print("3. Visa ställning i världscupen")
    print("4. Visa världsmästare")
    ...
    if choice == 1:
        ...
        skidskytt_id, skidskytt_namn, _, _ = skidskyttar[choice - 1]
        ...
        # Get the chosen tavling name and display the corresponding resultat table
        tavling_id, tavling_namn, tavling_gren, tavling_kon, topp_3 = tavlingar[choice - 1]
        print(f"\nResultat för tävling {tavling_namn} ({tavling_gren}) - (ID: {tavling_id}):")
        display_resultat(tavling_id, tavling_namn, tavling_kon, topp_3)

    elif choice == 3:
        # Prompt the user to choose a gender and display the corresponding vpoang standings
        while True:
            kon = input("Ange kön (m/f): ")
            if kon in ["m", "f"]:
                break
            print("Felaktigt kön. Tillåtna värden är 'm' och 'f'.")
        display_vpoang(kon)
        ...
        cursor.execute("""
             SELECT skidskyttar.namn, skidskyttar.nation, resultat.tavling, resultat.gren, resultat.kon, resultat.total_tid
        FROM resultat
        JOIN skidskyttar ON resultat.akarid = skidskyttar.id
        WHERE resultat.placering = 1 AND resultat.typ = 'VM'
        """)
        result = cursor.fetchall()

        # Create a PrettyTable instance with the column names
        columns = ["Namn", "Nation", "Tävling", "Gren", "Tid", "Kön"]
        table = PrettyTable(field_names=columns)

        # Add each world champion to the table as a row
        for row in result:
            total_tid = f"{int(row[5] // 60)}:{int(row[5] % 60):02d}"
            # Set the value of the "Kön" column based on the value of "kon"
            kon_column = "Damer" if row[4] == "f" else "Herrar"
            table.add_row([row[0], row[1], row[2], row[3], total_tid, kon_column])

        # Print the table
        print(table)

    elif choice == 5:
        # Exit the program and open index.py
        print("Huvudmenyn")
        os.system("index.py")
        break
